...
Kirigami.ApplicationWindow {
    ...
    function showPage(name) {
        if (name === root.page && pageStack.depth > 0) {
            return;
        }
        root.page = name;
        pageStack.clear();
        pageStack.push(name === "dock" ? dockPageComponent : themePageComponent);
    }

    pageStack.initialPage: startPage === "dock" ? dockPageComponent : themePageComponent
